...
import type { Trip } from "@/modules/trip.model";
...
export default function TourCategories() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(([tripTypes, trips]) => {
        if (cancelled) return;

        setTypes(tripTypes);
        setTripImages(buildTripImages(trips));
        setError(null);
      })
      .catch((err) => {
        if (cancelled) return;
        ...
        setError(
          err instanceof Error
            ? err.message
            : t(
                "categories.failedToLoad",
                "Failed to load trip types."
              )
        );
      })
      .finally(() => {
        ...
        >
          {loading && items.length === 0 && (
            <>
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={`cat-skel-${i}`} className="flex flex-col items-center flex-shrink-0 snap-center">
                  <div className={`w-36 h-48 sm:w-44 sm:h-56 lg:w-[185px] lg:h-[235px] rounded-[24px] sm:rounded-[28px] bg-gray-100 animate-pulse ${rotations[i % rotations.length]}`} />
                  <div className="w-24 h-5 mt-3 sm:mt-3.5 rounded-full bg-gray-100 animate-pulse" />
                </div>
              ))}
            </>
          )}
          ...
          {!loading && !error && items.length === 0 && (
            <p className="w-full text-center text-sm text-gray-500">
              {t(
                "categories.noTripTypes",
                "No trip types are available yet."
              )}
            </p>
          )}
